...
public class ModelTablePedido extends AbstractTableModel {
    ...
    private ArrayList<BeanPedido> data = new ArrayList();
    private ArrayList<TableModelListener> listeners = new ArrayList();        
    
    public ModelTablePedido() {        
        createHeaders();          
    }

    public ModelTablePedido(ArrayList<BeanPedido> lista) {
        data = lista;
        createHeaders();        
    }

    private void createHeaders() {
        headerTable.add("MODALIDAD");        
        headerTable.add("FECHA");
        headerTable.add("CLIENTE");
        headerTable.add("DIRECCION");     
        headerTable.add("TELEFONO");     
        headerTable.add("IMPORTE");     
        headerTable.add("PAGO");     
    }
    ...
    public void insertar(BeanPedido i) {
        data.add(i);
        TableModelEvent evento;
    ...
    public void update(BeanPedido i, int fila) {
        data.remove(fila);
        data.add(fila, i);
        TableModelEvent evento;
    ...
    public BeanPedido getValue(int rowIndex) {
        try {
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {        
        switch (columnIndex) {
            case 0:                                    
                return data.get(rowIndex).getModalidad();
            case 1:                
                 return data.get(rowIndex).getFecha();                   
            case 2:
                return data.get(rowIndex).getCliente();
            case 3:
            ...
            case 6:
                return data.get(rowIndex).getPago();        
            default:
                return null;
        }
    }
    ...
    public ArrayList<BeanPedido> getData() {
        return data;
    }
    ...
    public void setData(List<BeanPedido> listaProducto) {
        this.data = (ArrayList<BeanPedido>) listaProducto;        
    }
